...
import { generateGoogleCalendarUrl, downloadIcsFile, STEVAN_DOMAIN } from '../utils/calendar';
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 bg-white p-6 rounded-3xl border border-[#E7E0D5]">
        <div>
          ...
          <h1 className="text-2xl font-bold font-editorial text-[#2C2825] mt-1">
            My Scheduled Appointments & Invoices
          </h1>
          <p className="text-xs text-[#5C554E] mt-1">
            Track appointments, review price adjustments, sync with your calendar, and manage automated receipts.
          </p>
        </div>
      ...
      {filteredBookings.length === 0 ? (
        <div className="bg-white rounded-3xl border border-[#E7E0D5] p-12 text-center space-y-4">
          ...
          <p className="text-xs text-[#7A7168] max-w-md mx-auto">
            You don't have any appointments matching this view. Explore the Home Revival service catalogue to schedule Stevan.
          </p>
          <button
            onClick={() => setActiveTab('services')}
        ...
        <div className="space-y-6">
          {filteredBookings.map(booking => {
            ...
            return (
              ...
              >
                {/* Header row: Service Title, ID, Status, and Total */}
                ...
                {/* Scope / Price Adjustment Alert Banner (if adjusted) */}
                {latestAdjustment && (
                    ...
                    <div className="flex-1">
                      ...
                      {latestAdjustment.notes && (
                        <p className="text-[#7A7168] mt-0.5 italic">
                          Notes: {latestAdjustment.notes}
                        </p>
                      )}
                      <div className="mt-2 text-[11px] font-mono text-[#B25E29]">
                        Prior: ${latestAdjustment.previousTotal} → Adjusted Total: ${latestAdjustment.newTotal}
                      </div>
                    </div>
                  </div>
                )}

                {/* Details Grid: Schedule, Location, Line Items */}
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4 text-xs">
                  ...
                  {/* Column 2: Specifics Configured */}
                  <div className="bg-[#FAF7F2] p-4 rounded-2xl border border-[#EDE5D8] space-y-2">
                    <div className="font-semibold text-[#2C2825] flex items-center space-x-1.5">
                      <SlidersHorizontal className="w-4 h-4 text-[#B25E29]" />
                ...
                <div className="flex flex-wrap items-center justify-between gap-3 pt-3 border-t border-[#F0EBE1]">
                  ...
                  {/* Left: Push Reminder Controls & Calendar */}
                  <div className="flex flex-wrap items-center gap-2">
                    ...
                    <button
                      ...
                      title="Toggle push reminders for this booking"
                    >
                      <Bell className="w-3.5 h-3.5" />
                      <span>{booking.pushReminderEnabled ? 'Push Reminders Active' : 'Enable Reminders'}</span>
                    </button>
                    ...
                    <button
                      ...
                      title="Send a sample notification right now to preview how upcoming service reminders look"
                    >
                      Test Push Reminder
                    </button>
                  ...
                  {/* Right: Payment, Receipt & Price Adjustment Actions */}
                  <div className="flex flex-wrap items-center gap-2">
                    {/* Adjust specifics and price button */}
                    <button
                      ...
                      title="Adjust scope, specific parameters, or add materials items"
                    >
                    ...
                    {hasUnpaidBalance ? (
                      ...
                      >
                        ...
                        <span>Pay Balance (${remainingBalance})</span>
                      </button>
                    ) : (
